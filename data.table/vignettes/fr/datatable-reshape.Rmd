...
Cette vignette traite de l'utilisation par défaut des fonctions de restructuration `melt` (du format large au long) et `dcast` (du format long à large) pour les *data.tables* ainsi que des **nouvelles fonctionnalités étendues** de fusion (melting) et dispersion (casting) sur *plusieurs colonnes* disponibles depuis la version `v1.9.6`.
...
1. Examiner brievement l'utilisation par défaut des fonctions `melt` et `dcast` sur les data.tables pour les convertir du format *large* au format *long* et *vice versa*
...
### a) fusionner (`melt`) des colonnes dans un `data.table` (format large au long)
...
* Nous pouvons également spécifier des *indices* au lieu des *noms* de colonne.
...
### b) disperser (`dcast`) des lignes (format long au large)
...
En d'autres termes, nous aimerions fusionner toutes les observations *enfants* correspondant à chaque `family_id, age_mother` dans la même ligne. Nous pouvons le faire en utilisant la fonction `dcast` comme suit :

```{r}
dcast(DT.m1, family_id + age_mother ~ child, value.var = "dob")
```

* `dcast` utilise la notation *formule* ( *formula* ). Les variables du côté gauche (*LHS*) de la formule correspondent aux variables *id* et sur le côté droit (*RHS*) aux variables *measure*.
...
Vous pouvez également passer une fonction d'aggrégation dans `dcast` avec l'argument `fun.aggregate`. Ceci est particulièrement essentiel lorsque la formule fournie ne permet pas d'identifier une seule observation pour chaque cellule.
...
1. Ce que nous voulions faire était de combiner toutes les colonnes de type `dob` ensemble, et toutes les colonnes de type `gender` ensemble. Au lieu de cela, nous combinons tout, puis nous les scindons à nouveau. Il est facile de voir que c'est une approche détournée (et inefficace).
...
4. Enfin, transformons le jeu de données. Mais le problème est qu'il s'agit d'une opération beaucoup plus coûteuse en calcul que *melt*. En particulier, il faut calculer l'ordre des variables dans la formule, ce qui est coûteux.
...
Si, comme dans les données ci-dessus, les colonnes d'entrée à fusionner ont des noms réguliers, alors nous pouvons utiliser `measure`, qui permet de spécifier les colonnes à fusionner via un séparateur ou une expression régulière. Par exemple, considérons les données `iris`,

```{r}
(two.iris = data.table(datasets::iris)[c(1,150)])
```

Les données iris possèdent quatre colonnes numériques avec une structure régulière : d'abord la partie de la fleur, suivie d'un point, puis le type de mesure. Pour spécifier que nous voulons fusionner ces quatre colonnes, nous pouvons utiliser `measure` avec `sep="."` ce qui signifie utiliser `strsplit` sur tous les noms de colonnes ; les colonnes qui résultent en un nombre maximum de groupes après division seront utilisées comme `measure.vars` :
...
Dans le code ci-dessus, nous avons utilisé `sep="_child"`, ce qui entraîne la fusion uniquement des colonnes contenant cette chaîne (six noms de colonnes séparés en deux groupes chacun). L'argument `child=as.integer` signifie que le second groupe donnera lieu à une colonne de sortie nommée `child` avec des valeurs définies en appliquant la fonction `as.integer` aux chaînes de caractères de ce groupe.
...
Parfait ! Nous pouvons maintenant fusionner (`melt`) plusieurs colonnes simultanément. Maintenant, étant donné le jeu de données `DT.m2`, comment pouvons-nous revenir au même format que le jeu de données avec lequel nous avons commencé ?
...
#### - Dispersion (`cast`) de plusieurs `value.var`s simultanément
